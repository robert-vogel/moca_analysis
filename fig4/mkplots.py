...
def read_data(fname):
    ...
    with open(fname, "r") as fid:
        ...
        for i, tline in enumerate(fid):
            ...
            if (re.match("^[0-1]*\.?[0-1]*E?e?-?[0-1]*$",tline[label_col_idx])
                is None):
...
def weights_by_cv(data, labels, kfolds, stratified_cv, seed):
    ...
    for train, test in cv_generator:
        cl_umoca = cls.Umoca(max_iter=UMOCA_MAX_ITER,tol=UMOCA_TOL)
...
def auc_by_cv(data, labels, kfolds, stratified_cv, seed):
    ...
    for i, cl in enumerate([cls.Umoca(max_iter=UMOCA_MAX_ITER, tol=UMOCA_TOL), cls.Woc(), Gmm()]):
